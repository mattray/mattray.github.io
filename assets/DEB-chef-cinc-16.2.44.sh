...
# Chef 16.2.44
cd
rm -rf ~/chef-16.2.44 ~/v16.2.44.tar.gz
sudo rm -rf /opt/chef
sudo mkdir /opt/chef
sudo chown omnibus:omnibus -R /opt/chef
wget https://github.com/chef/chef/archive/v16.2.44.tar.gz
tar -xzf v16.2.44.tar.gz
cd ~/chef-16.2.44/omnibus/
bundle config set without 'development'
bundle install --path=.bundle
bundle exec omnibus build chef -l internal
cp ~/chef-16.2.44/omnibus/pkg/chef*deb ~/
...
git clone -q -b v16.2.44 https://github.com/chef/chef.git
# patch patch.sh
sed -e '/^source/ s/^/# /' patch.sh > patch2.sh
sed -e '/^rm/,+3 s/^/# /' patch2.sh > patch3.sh
bash patch3.sh 16.2.44
cd chef/omnibus/
bundle config set without 'development'
bundle install --path=.bundle
