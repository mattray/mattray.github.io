...
# Chef 15.15.0
cd
rm -rf ~/chef-15.15.0 ~/v15.15.0.tar.gz
sudo rm -rf /opt/chef
sudo mkdir /opt/chef
sudo chown omnibus:omnibus -R /opt/chef
wget https://github.com/chef/chef/archive/v15.15.0.tar.gz
tar -xzf v15.15.0.tar.gz
cd ~/chef-15.15.0/omnibus/
bundle config set without 'development docs debug'
bundle install --path=.bundle
bundle exec omnibus build chef -l internal
cp ~/chef-15.15.0/omnibus/pkg/chef*deb ~/
cp ~/chef-15.15.0/omnibus/pkg/chef*deb /tmp/

# Cinc 15.15.0
cd
rm -rf ~/cinc-full-15.15.0 ~/cinc-full-15.15.0.tar.xz
sudo rm -rf /opt/chef /opt/cinc
sudo mkdir /opt/cinc
sudo chown omnibus:omnibus -R /opt/cinc
curl http://downloads.cinc.sh/source/stable/cinc/cinc-full-15.15.0.tar.xz --output cinc-full-15.15.0.tar.xz
tar -xJf cinc-full-15.15.0.tar.xz
cd cinc-full-15.15.0/cinc-15.15.0/omnibus/
bundle lock --update=chef
bundle config set without 'development docs debug'
bundle install --path=.bundle
bundle exec omnibus build cinc -l internal
cp ~/cinc-full-15.15.0/cinc-15.15.0/omnibus/pkg/cinc*deb ~/
cp ~/cinc-full-15.15.0/cinc-15.15.0/omnibus/pkg/cinc*deb /tmp/

chmod 644 /tmp/*deb

echo "15.15.0 Complete!"
date
