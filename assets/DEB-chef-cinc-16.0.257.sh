...
# Chef 16.0.257
cd
rm -rf ~/chef-16.0.257 ~/v16.0.257.tar.gz
sudo rm -rf /opt/chef
sudo mkdir /opt/chef
sudo chown omnibus:omnibus -R /opt/chef
wget https://github.com/chef/chef/archive/v16.0.257.tar.gz
tar -xzf v16.0.257.tar.gz
cd ~/chef-16.0.257/omnibus/
bundle config set without 'development'
bundle install --path=.bundle
bundle exec omnibus build chef -l internal
cp ~/chef-16.0.257/omnibus/pkg/chef*deb ~/
...
git clone -q -b v16.0.257 https://github.com/chef/chef.git
# patch patch.sh
sed -e '/^source/ s/^/# /' patch.sh > patch2.sh
sed -e '/^rm/,+3 s/^/# /' patch2.sh > patch3.sh
bash patch3.sh 16.0.257
cd chef/omnibus/
bundle config set without 'development'
bundle install --path=.bundle
