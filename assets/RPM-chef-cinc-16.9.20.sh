...
# Chef 16.9.20
cd
rm -rf ~/chef-16.9.20 ~/v16.9.20.tar.gz
sudo rm -rf /opt/chef
sudo mkdir /opt/chef
sudo chown omnibus:omnibus -R /opt/chef
wget https://github.com/chef/chef/archive/v16.9.20.tar.gz
tar -xzf v16.9.20.tar.gz
cd ~/chef-16.9.20/omnibus/
bundle config set without 'development docs debug'
bundle install --path=.bundle
bundle exec omnibus build chef -l internal
cp ~/chef-16.9.20/omnibus/pkg/chef*rpm ~/
cp ~/chef-16.9.20/omnibus/pkg/chef*rpm /tmp/

# Cinc 16.9.20
cd
rm -rf ~/cinc-full-16.9.20 ~/cinc-full-16.9.20.tar.xz
sudo rm -rf /opt/chef /opt/cinc
sudo mkdir /opt/cinc
sudo chown omnibus:omnibus -R /opt/cinc
curl http://downloads.cinc.sh/source/stable/cinc/cinc-full-16.9.20.tar.xz --output cinc-full-16.9.20.tar.xz
tar -xJf cinc-full-16.9.20.tar.xz
cd cinc-full-16.9.20/cinc-16.9.20/omnibus/
bundle lock --update=chef
bundle config set without 'development docs debug'
bundle install --path=.bundle
bundle exec omnibus build cinc -l internal
cp ~/cinc-full-16.9.20/cinc-16.9.20/omnibus/pkg/cinc*rpm ~/
cp ~/cinc-full-16.9.20/cinc-16.9.20/omnibus/pkg/cinc*rpm /tmp/

chmod 644 /tmp/*rpm

echo "16.9.20 Complete!"
date
